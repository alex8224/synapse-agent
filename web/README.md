...
- **两个维度**：卡片右上角可在「按周」与「按天」之间切换（选择记在 `usagePrefs`）。「按周」是
  上述 52 周日历；「按天」是小时矩阵——窗口末最多 14 天，每天一行、24 列 UTC 小时，行连续
  （安静的一天是一行 0），超出 14 天的部分由 `heatmap.hourly.truncated` 标记并在副标题说明。
  单日区间因此正好是 24 个格子。小时矩阵使用**自己的一套四分位阈值**（一小时与一整天不可比），
  悬停给出 `2026年9月21日（周一） 14:00–15:00 UTC` 与该小时 Token / 会话。
