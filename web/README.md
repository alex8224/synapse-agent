...
新会话一经创建，立即清空上一会话的累计 token 和上下文用量显示；即使运行时初始化
仍在等待或失败，也不会沿用旧值。累计用量只在该会话的 `runtime.session.open` 回应后恢复。
用量显示残留与实际对话上下文串台是不同问题，不能仅凭底栏数字判断上下文已共享。
守护进程重启后，失去 worker 的工作流会显示为结果不确定，而不是继续显示正常运行中。
它仍会暂停同一项目所有会话的新消息；在工作流面板点击「取消工作流」可解除占用，
但不能撤销可能已经发生的修改。若消息提交被明确拒绝，输入内容会保留供再次发送。
...
列表占满宽度，选中文件后切到详情并显示「返回文件列表」；标题栏收起文件列表时**切到详情**（而不是列表与详情
同时隐藏的空白视图），展开时切回列表。轮次导轨在手机上隐藏（正文太窄）。
...
- 左列 `SideBar` 占满整个视口高度：导航（新建任务 `Ctrl+N`、搜索 `Ctrl+K`，与折叠轨上
  ...
  运行时诊断、主题切换、退出配对、使用统计、设置；会话信息不在这里，它由顶栏标题打开）。
  折叠成 44px 轨后**入口一个不少**：上下文操作改成竖排，用量与设置图标照旧留在底部（只是
  收起标签），所以折叠只隐藏会话树与文字，不会隐藏任何一个入口。主题切换是一键的：
  图标画的是**将要切换到的**那个主题（浅色画月亮、深色画太阳），`Ctrl+Shift+L` 在任何位置
  等效，「跟随系统」仍留在设置对话框的三档控件里。
...
（`src/components/composer/actions/ActionMenu.tsx`），宿主只拥有所有行共有的规则：触发器、开/关、
键盘契约（触发器上 Enter/Space 打开、方向键在行间移动并环绕、Home/End 跳到首/末行、Esc 关闭并把
焦点交回触发器）、外点关闭与激活保护。菜单**向上弹出**（输入卡在窗口底部）并经 `FloatingPanel`
portal 到 `document.body`，所以卡片的亚克力层裁不到它，行也不在 `<form>` 里——点行绝不会提交。
每个动作是 `src/components/composer/actions/*Action.ts` 里的一个 `ComposerActionDefinition`，
**新增动作 = 一个模块 + `manifest.ts` 里一行**，没有可变注册中心、外部插件或用户布局文件；
动作拿到的是一个**类型化的窄 context**（`pickImages` / `startWindowScreenshot` /
`openScreenshotSettings`），永远拿不到 store。
首批三个动作：**添加图片**（复用卡片自己的隐藏 `<input>` 与 `handleFiles`，与粘贴/拖放同一条上传
路径）、**窗口截图**（排队一个异步截图任务）与 **截图设置**（打开截图工具自身的设置窗口）。后两者
只把动作交给运行时：卡片本身**没有截图分支**（进度、取消、结果都在独立的 `ScreenshotTaskBanner`
里），前端也从不直连截图工具。
契约与注册表规则由 `tests/composerActions.test.ts` 守护，真实浏览器验收在
`tests/composerActions.verify.ts`（菜单开合与向上弹出、方向键/Home/End、Esc 还焦点、外点关闭、
截图行可执行且点击经运行时排队/打开、绝不提交本轮、图片行打开 file picker、添加项目按钮保留、
窄屏仍在视口内）。
...
diff 视图（`GitDiffView.tsx`）的顶部工具栏把超长路径截断（`title` 里给全文），把「统一／分栏」
切换与文件头按钮留在右侧；配色只走主题角色（`data-theme` 换肤，**不再**依赖永不生效的 `dark:*`）。
`\ No newline at end of file` 不再丢：它折叠为所属行上的「⏎ 缺少末尾换行符」徽标。分栏视图两侧
固定各占一半（`w-1/2 min-w-0`），增删行数不等时用空白占位对齐，长行自动换行（含连续无空格文本），
两侧仍逐行对齐；统一视图保留长行并在代码区横向滚动，不会把工具栏挤出窗口。
标题栏按钮的 SVG 图标与按钮空白区域均可点击，不触发窗口拖动。
...
侧栏底部操作行有 `DataUsage` 入口（与设置入口同排；折叠轨上同样保留这个图标），打开 `UsageDashboardDialog`
...
- **口径**：累计吞吐 `total_tokens = provider_input + output`（provider 处理过的全部 prompt
  ...
  不按 5000 截断）；自定义区间最长 3660 天；后端仍以 366 天为上限截断热力数据（累计不受影响），
  而「Token 活动与工程投入矩阵」固定画成以区间末日对齐的 52 周（364 天）日历网格，无数据格子
  只留底色、未来日期不绘制。月份刻度锚定在它标注的那一列（按每周周四归属月份），因此刻度与
  格子永远对齐；四档强度取窗口内**有活动日期**的四分位阈值（最高一天恒为最深档），单个峰值
  不会把其余日期压到最低档；悬停给出完整日期（`2026年9月21日（周一）`）与当日 Token / 会话。
- **两个维度**：卡片右上角可在「按周」与「按天」之间切换（选择记在 `usagePrefs`）。「按周」是
  上述 52 周日历（UTC 日）；「按天」是小时矩阵——**按宿主本地时区**聚合（载荷带
  `heatmap.hourly.timezone`，如 `UTC+08:00`），显示最近最多 14 个**有活动的本地日**，每天一行、
  24 列本地小时（行自带日期，因此不补空行）。更多活跃日由 `heatmap.hourly.truncated` 标记并在
  副标题说明。小时矩阵使用**自己的一套四分位阈值**（一小时与一整天不可比），悬停给出
  `2026年9月21日（周一） 14:00–15:00（UTC+08:00）` 与该小时 Token / 会话。
- **真实数据**：热力图按真实日期记录并可切换 Token / 会话（`代码变更行数` 指标无数据来源，逐格
  提示 `—`）；左侧趋势为分层堆叠面积图（缓存命中 / 未命中输入 / 正文输出 + 未压缩基线虚线），
  与「按 Token 排序」的会话排行都来自真实事件；项目下拉来自宿主 catalog 的可见项目，选择项目或
  「全部」都会读取对应工作区数据，浏览器不能提交任意路径，未知 `project` 返回 400。
- **无来源即留白**：费用 / 代码行数 / 工具平均耗时显示 `—`；Agent/角色维度停用；不展示任何
  无数据来源的卡片或列（工具输出压缩记录、代码变更行数、工程能效比均已移除）。
- **交互**：单一 effect 触发请求；自定义日期点「筛选」后才查询；`AbortController` + stale 保护；
  ...
  支持按「可用模型」下钻过滤各项用量指标（该记忆由 `src/stores/usagePrefs.ts` 持有，是
  `tests/sourceGuard.test.ts` 允许触达浏览器存储的三个非机密 store 之一）。
...
主题的入口有两个：侧栏底部的切换按钮（一键）与 `Ctrl+Shift+L`（`App.tsx` 的全局按键）。
两者是同一个动作，也共用 `consoleShortcuts.ts` 里的同一串 chord——按钮 tooltip 里的
「(Ctrl + Shift + L)」与 F1 列表都取自那一张表。切换的是**眼前看到的**配色：偏好仍是
「跟随系统」时，点击先解析当前系统主题、再把相反的那个存成显式选择（`oppositeAppearance`），
所以按钮永远会改变画面，而不是存下系统已经在用的那一个；`tests/appearance.test.ts` 钉住这条
语义，`tests/fluentAppearance.verify.ts` 在真实浏览器里验收按钮与快捷键两条路径。
...
「窗口截图」在运行时**异步**截取一个窗口，完成后把每一帧变成**普通附件**加入输入框——截图结果
**绝不会自动发送**。前端从不直连截图工具：它只调用运行时，运行时再通过 CLI 驱动本机的
`windows-capture` 工具。

| 操作 | wire 方法 | 说明 |
|---|---|---|
| 能力查询 | `runtime.screenshot.status` | 只读：报告截图工具是否可用（非 Windows 或未构建时明确不可用并给出原因）、是否忙、以及当前会话截图任务的状态/进度/已 finalize 的附件 id。能力探测只运行工具的 `--version`，**不会启动 GUI** |
| 打开设置 | `runtime.screenshot.settings.open` | 打开（或聚焦）截图工具自身的设置窗口（选择窗口、帧数、间隔等）。不截图、不写附件 |
| 开始截图 | `runtime.screenshot.capture` | 排队一个异步任务，立即返回任务快照。控制台默认**不覆盖**参数，让工具用它自己保存的配置（`override > saved > default`，参数在「截图设置」里选）；任务在后台轮询工具、按块读取结果并经**既有附件 store**（begin/append/finish）finalize，帧数按输入框预算封顶。已有任务排队/运行中时该调用**幂等**（返回正在运行的任务），不会重复起任务 |
| 取消 | `runtime.screenshot.cancel` | 请求取消（幂等）；工具被要求停止，任务落到明确的终态 |

无目标窗口时工具会打开自己的选择界面并返回 `target_required`，控制台据此提示「请选择窗口后重试」，
并提供「重试」。取消、失败、工具退出都落到**命名状态**（`cancelled` / `failed` / `target_required`），
绝不假装成功。每个任务绑定**发起时的会话与草稿世代**：切回原会话且尚未发送新草稿时，结果自动加入
输入框；否则结果**保留为可见的待确认项**，提供「加入输入框」/「丢弃」两条明确路径，绝不塞进新草稿。

只有全部图片校验、入库成功后，运行时才发布 `completed`；工具自身完成抓帧时，控制台仍可能显示

对仍会**提前**发布 `completed`（附件尚未回填，或只回填了一部分）的旧 daemon，控制台不会立刻报错：
「正在回填截图附件…」。页面刷新或重连会重新读取已有任务：无法确认原草稿身份时，显示
「截图已完成」及「加入输入框」，不会重新采集或自动发送；重复刷新不会重复加入附件。
进度卡的「刷新状态」可手动同步。恢复本次结果前不要重启 daemon（任务快照目前只保留在 daemon 内存中）。

截图任务的状态与进度由入口内的 `src/stores/screenshotTask.ts` 持有（不进 `useConsoleStore`），
进度/取消/待确认结果由 `src/components/ScreenshotTaskBanner.tsx` 绘制在输入卡片**上方**——输入卡片
本身没有截图分支。数量与大小沿用附件规则（每次最多 8 张、每张 ≤ 4 MB），开始前检查剩余名额、完成后
再次按上限截断。纯解码与展示规则在 `src/runtime-client/screenshot.ts`，测试见
`tests/screenshot.test.ts`。

截图行是**已 finalize 的附件**：输入框里的预览按 `attachment_id` 经历史缩略图 loader 加载（没有本地
`File`），因此 `PendingAttachment.source` 现在是可选的——本地上传的行仍用本地 blob，截图行用 id 预览。
缩略图与**悬停放大预览**共用同一个按 id 取字节的 hook（`src/components/useAttachmentResource.ts`，
primitive 依赖 + 异步世代隔离），所以重渲染不会重复读取，悬停也能显示真实大图。

## 添加项目（输入区「添加项目」按钮）

控制行左侧的「添加项目」按钮（与动作菜单并排的独立按钮）打开该对话框：浏览器无法解析宿主路径，
...
图片附件除**粘贴/拖放**外，也可用控制行左下动作菜单的「添加图片」行点选（`+` 现在是通用动作菜单，
图片只是其中一行）；三条入口共用同一校验与上传管线，8 张 / 4 MB 上限与行内 Pill + 悬停预览行为一致。
