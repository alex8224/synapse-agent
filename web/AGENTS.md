- A turn's change cards are the runtime's, not the browser's: `runtime/workspace_changes.py` snapshots the workspace before and after the turn (read-only git) and reports that turn's own per-file line counts, which the daemon persists as a `changes` transcript event and emits live as `turn_changes` at settlement. The console only paints them (`transcriptRows/ChangesRow.tsx`), so nothing here may recompute them from `runtime.git.status` -- that is the workspace's standing delta against `HEAD`, which counts every earlier turn again.
- Undoing one file of one turn is the runtime's write, not the browser's: `runtime.workspace.revert` restores that single path from the copy `runtime/turn_reverts.py` kept before the turn (under the workspace's own `.synapse/turn-snapshots/`), refuses while a turn is running or once the file has moved on, and never touches `HEAD`, the index or any other file. The console only asks and repaints (`ChangesRow.tsx` -> `useConsoleStore.revertTurnChange`), and it learns the reverted state from the history read (`HistoryEvent.reverted_paths`) or from its own successful call -- never by recomputing anything from `git.status`, and never by editing a projected transcript row.
...
- The transcript is the same shape for rows: a row kind is one `src/components/transcriptRows/*.tsx` module plus one line in that directory's `registry.tsx` (a `Record<TranscriptMessage['type'], RowRenderer>`), and how it folds is one line in `src/stores/transcriptRowPolicy.ts`. `Transcript.tsx` keeps only the scroller, the virtualizer and the dispatcher, so no row kind is special-cased by position there. Guards read the row directory through `tests/helpers/transcriptSource.ts` instead of one component, and layout order that only a browser can measure (where a rule sits, how far apart two rows land) is pinned in `tests/transcriptFoldSpace.verify.ts`.
