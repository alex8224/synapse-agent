...
class _FakeRun:
    def __init__(self, call_id: str, status: str) -> None:
...
class _FakeMonitor:
    ...
    def snapshot(self):
        return None, self._runs


def _monitor_app(runs: list[Any]) -> tuple[TranscriptController, _FakeApp]:
    controller, app = _make()
    app._subagent_monitor = _FakeMonitor(runs)
    app._subagent_monitor_auto_opened = False
    app.monitor_opens = 0
    app._open_subagent_monitor = lambda: setattr(app, "monitor_opens", app.monitor_opens + 1)
    return controller, app


def test_suppress_dag_group_requires_tracked_tasks() -> None:
    controller, app = _monitor_app(
        [_FakeRun("run-1", "running"), _FakeRun("run-2", "pending")]
    )

    assert controller.should_suppress_dag_task_tool_group([]) is False
    assert controller.should_suppress_dag_task_tool_group([{"name": "bash"}]) is False
    assert (
        controller.should_suppress_dag_task_tool_group(
            [{"name": "task", "id": "run-1"}, {"name": "task", "id": "run-2"}]
        )
        is True
    )
    assert (
        controller.should_suppress_dag_task_tool_group(
            [{"name": "task", "id": "run-1"}, {"name": "task", "id": "run-9"}]
        )
        is False
    )


def test_suppress_dag_group_without_monitor_is_false() -> None:
    controller, app = _make()
    app._subagent_monitor = None

    assert (
        controller.should_suppress_dag_task_tool_group([{"name": "task", "id": "x"}])
        is False
    )


def test_sync_subagent_monitor_block_auto_opens_once() -> None:
    controller, app = _monitor_app([_FakeRun("run-1", "running")])

    controller.sync_subagent_monitor_block()
    assert app.monitor_opens == 1
    assert app._subagent_monitor_auto_opened is True

    # Second detection in the same turn does not reopen the dialog.
    controller.sync_subagent_monitor_block()
    assert app.monitor_opens == 1


def test_sync_subagent_monitor_block_force_refreshes() -> None:
    controller, app = _monitor_app([_FakeRun("run-1", "running")])
    app._subagent_monitor_auto_opened = True

    controller.sync_subagent_monitor_block(force=True)
    assert app.monitor_opens == 1


def test_sync_subagent_monitor_block_skips_empty_runs() -> None:
    controller, app = _monitor_app([])

    controller.sync_subagent_monitor_block()
    assert app.monitor_opens == 0
