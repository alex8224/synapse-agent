...
| **TopBar (顶栏)** | 工作区列顶部的单行 chip，**三轨栅格**（两侧等宽 `1fr`）：左轨 [\|] 侧栏折叠开关 · 项目 · ⎇ Git 分支（含 ↑/↓ 跟踪计数）；中轨为**会话标题**，因两侧等宽而**真正居中于工作区列**（不随左右 chip 宽度漂移，独立轨道也保证窄屏不重叠）；右轨为最右变更统计（脏/干净状态点 + `+N -M` **真实 tracked 增删行数**）；**分支与统计两个 chip 都是按钮、都可打开只读 Git Explorer**；窄屏（< `lg`）隐藏次要的项目 chip，保留标题与分支；工作区路径移至侧栏底部身份行 |
...
| **CommandBar (输入区)** | 固定在工作区列底部、与聊天列同宽同边（`.console-gutter` + `.console-column`）的卡片：文本在上，附件在左下，模型 / 推理强度 / 发送在右下；若有正在运行的任务则浮现 Steer queue 状态 |
...
| TopBar | 右侧变更统计 | 有（脏/干净状态点 + `runtime.git.status` 的**真实 tracked 增删行数** `+N -M`，不是文件数；**点击打开只读 Git Explorer**，与分支 chip 同为入口；行数未知（git 无法作答）时只显示状态点、不渲染数字，避免虚构的 `+0 -0`） | 已实现（本轮改为行数） |
