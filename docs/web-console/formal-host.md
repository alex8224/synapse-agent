...
- **读取走窄投影表**：一次压缩事件的 `event_json` 约 30–90 KB（`live_zone_plan`、
  `wire_fingerprints` 等诊断占绝大部分），而面板只需要十个标量。生产者因此在同一事务里把
  这些标量写进 `model_request_compression_rollup`（约 120 B/行），面板优先读它——一个
  30 天窗口从 ~20 s 降到 ~0.4 s。投影是**派生数据**，事件表仍是唯一事实来源；面板用两张表的
  `COUNT(*)`（都走索引）判断投影是否完整，不完整时对**尚未投影**的事件回退到
  `json_extract`（`json_valid` 过滤畸形行），因此升级后不会少算、也不会重复计数。
  升级前写入的事件由 `ToolOutputRepository` 在打开时按游标分批补齐（每批最多 4000 行），
  积压较大时在后台线程补齐，不阻塞调用方；补齐期间面板仍给出正确结果，只是慢一些。
- 热力图是**连续日历日**序列（无活动日为 0，保证星期对齐）：`all` 取首末活动日之间连续，
  ...
  **窗口末 366 天**，并以载荷 `heatmap.truncated` + UI 文案标明——这只截断热力图，**累计
  KPI 仍覆盖整个窗口**。
- `heatmap.hourly` 是同一窗口的**小时分辨率**视图，也是**唯一按本地时区（而非 UTC）聚合**的部分：
  每天一行、固定 24 列（下标即本地小时），只显示最近最多 **14 个有活动的本地日**
  （`MAX_HEATMAP_HOUR_DAYS`），行自带日期所以不补空行；`heatmap.hourly.truncated` 标记还有更早的
  活跃日未显示，`heatmap.hourly.timezone` 给出宿主当前偏移（如 `UTC+08:00`，零偏移为 `UTC`）。
  同样是**只截断矩阵，不动累计**（其余面板仍是 UTC 闭区间）。
  实现上事件按 **UTC** `(日期, 小时)` 建桶、只保留最近两周多一点的桶，发射载荷时再按当天偏移
  整体平移（偏移只会整体挪动一个小时的桶，不会切开它）——比逐事件做时区转换快一个数量级。
  多项目合并时 thread_id 按项目命名空间隔离，因此同一 thread_id 出现在两个工作区时该小时计 2 个会话。
