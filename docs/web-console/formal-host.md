...
- 热力图是**连续日历日**序列（无活动日为 0，保证星期对齐）：`all` 取首末活动日之间连续，
  ...
  **窗口末 366 天**，并以载荷 `heatmap.truncated` + UI 文案标明——这只截断热力图，**累计
  KPI 仍覆盖整个窗口**。
- `heatmap.hourly` 是同一窗口的**小时分辨率**视图：窗口末最多 **14 天**（`MAX_HEATMAP_HOUR_DAYS`），
  每天一行、固定 24 列（下标即 UTC 小时），行连续（安静的一天是一行 0，不会因缺行错位）。
  `heatmap.hourly.truncated` 标记窗口还有更早的日期未显示；同样是**只截断矩阵，不动累计**。
  聚合按 `(日期, 小时)` 建桶并只保留最近 14 天，多项目合并时把 thread_id 按项目命名空间隔离，
  因此同一 thread_id 出现在两个工作区时，该小时计 2 个会话。
