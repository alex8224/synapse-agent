...
- **会话管理**：`runtime.session.create` / `rename` / `delete` / `search`（各自独立 capability）；`create` 由服务端分配身份；`delete` 删元数据行与 thread goal 并把该 thread 从 checkpoint、transcript projection、全文检索索引与回滚快照里一并清除，结果里的 `retained_history` 为 `true` 仅当有存储拒绝（`purge_failures` 点名）；`search` 是**元数据搜索**而非全文检索。
