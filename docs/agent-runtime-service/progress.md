...
- **删除会话 = 真删（本轮改动）**：`runtime.session.delete` 现在删元数据行与 thread goal，**并**把该 thread 从它留下的每一个存储里清掉——LangGraph checkpoint（含 `tools:*` 子代理命名空间）、transcript projection、全文检索索引、以及 `.synapse/turn-snapshots/<thread_id>/` 的回滚快照（`synapse.sessions.thread_purge.purge_thread`）。**此前的语义（只删元数据行与 thread goal，checkpoint 与 transcript projection 保留，`retained_history` 恒为 `true`）是错的**：会话虽从列表消失，却仍能被关键字搜到——`search_session` 的全文分支只查 `search-index.sqlite`，从不查 `sessions` 表，所以「已删除」的会话会重新出现在搜索结果里（这正是用户报的「删了又冒出来」）。现在 `retained_history` 如实报告结果：`false` = 四个存储都已清空，`true` = 有存储拒绝且 `purge_failures` 点名（未定位到的存储同样算失败——「不知道它在哪」不等于「它已清空」），因此 UI 只在 `false` 时才可以声称对话已被擦除。清理与行删除在同一个 lifecycle 临界区里完成（`RuntimeManager.delete_session_ref` 新增 `purge_history` 回调，`asyncio.to_thread` 执行），并发 open/submit 无法复活会话；CLI `sessions delete` 与 ACP `session/delete` 走同一套清理，另加 `sessions purge [--apply]` 清理历史遗留的孤儿（元数据已删、历史仍在的 thread；元数据表读不出来时不列出任何候选，避免把全部会话当孤儿删掉）。证据：`tests/test_thread_purge.py`（四个存储 + 子代理命名空间 + 幂等 + 不定位/打不开的存储被点名 + `..` 等危险 id 被拒 + 孤儿扫描在元数据缺失时返回空）、`tests/test_runtime_session_management.py::test_delete_rejects_busy_runtime_and_purges_the_conversation`（另一个会话不受影响）、`::test_delete_names_a_store_it_could_not_purge`、`tests/test_cli.py::test_cli_sessions_delete_erases_the_conversation`、`::test_cli_sessions_purge_lists_orphans_and_only_erases_with_apply`；契约已重新导出（`DeleteSessionResult.purge_failures`），`transport/client.py` 校验两个字段必须一致。

- **`retained history` 与附件**：附件的 durable 引用存活在 projection 中，`read_session_history` 的 `HistoryEvent.attachments` 只带元数据（不含 base64），字节由 `runtime.attachments.read` 按需分块读取；projection 重建（`synapse.sessions.transcript._message_attachment_refs`）与 append 路径（`synapse.runtime.sessions.persistence._durable_attachment_refs`）从同一处 message metadata 重新导出 refs，因此重建结果与写入结果一致。删除会话会一并清掉该 thread 的 projection，所以它的附件引用也随之消失（附件字节本身按 `attachment_store` 的既有生命周期管理，不在本次改动范围内）。
