...
| 2026-08-08 | 侧栏 UI 精修 | 半宽 26 列（52→26）+ 字符串截断缩短（标题 14/项目名 16）；高度避开 topbar 与输入区（`height:100%` + `margin-top:2` + `margin-bottom:3`，Textual 无 calc）；项目图标 📁 + 会话数精简为数字；状态符号+颜色（●/◐/⏸/✕/○）；当前会话 ◆ 标记；项目默认折叠最近 5 个会话 + `▸ 展开显示 (N more)` 展开全部（记忆展开状态）；j/k、Home/End、g/G 导航；`_rebuild_tree` 统一重建并保留光标；修复无 by_project provider 时全局状态 thread 误合并进每个项目 | 新增折叠/展开测试 2 个；渲染实验（proto）验证尺寸与符号 | 全量 1118 passed、2 skipped；仅 2 个既有可选原生 tool-output transformer 断言失败 |
| 2026-08-08 | 切换断链 bug 修复 | 会话切走再切回后 transcript 冻结（一直 thinking 但内容不渲染）：`attach()` 在 UI 线程执行时 replay broker 历史触发 `bridge.emit` → `call_from_thread` 抛 RuntimeError（UI 线程调用非法）→ bridge 被关闭，后续 live 事件全部丢弃。修复：`TextualTurnEventBridge._wake` 线程感知——UI 线程调用时同步 `drain()`（安全，已在线程上），其他线程走 `call_from_thread`；`drain` 的 reschedule 分支同样复用。三省优化：`apply_effects` 切换探针改用 `runtime_for`（不再 attach→detach 导致 replay 脏渲染 + 白建 bridge），渲染 attach 只发生一次（transcript reset 完成后） | 新增回归测试 `test_switch_back_keeps_bridge_alive`（复现 + 断言 live 事件到达 renderer）；3 个 dialogs 切换测试更新为新探针语义 | 全量 1119 passed、2 skipped；仅 2 个既有可选原生 tool-output transformer 断言失败 |
| 2026-08-08 | 切回后 turn 轮换内容丢失修复 | 切回后"turn 开始的内容（思考/工具）没渲染、后续调用才渲染"：切走期间 turn 可能结束/轮换，attach 的 replay 事件被 renderer 的 `turn_id` 门控过滤（绑定新 active turn），旧 turn 事件全部丢弃，restore 投影又可能未包含刚完成的 turn。修复：renderer 新增 `replay(event)` 路径（不过滤 turn_id，仅 thread_id + sequence 有序），bridge 新增 `replay`，attach 的 replay 循环走该路径；live 事件仍走 `emit`（turn_id 门控防跨 turn 污染）。安全性：broker 保留 `TURN_COMPLETED`（lossless）提供 turn 边界，sink 可正确收尾；实测 debug 确认 replay 的 sink 调用全部执行 | 新增单元测试 `test_renderer_replay_bypasses_turn_id_gate`（emit 过滤 vs replay 消费） | 全量 1120 passed、2 skipped；仅 2 个既有可选原生 tool-output transformer 断言失败 |
