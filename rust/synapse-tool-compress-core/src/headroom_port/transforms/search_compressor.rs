...
//
// Derived from Headroom (Copyright 2025 Headroom Contributors) under Apache-2.0.
// Modified for Synapse; see ../../LICENSE and ../../NOTICE.
...
//!    [`crate::headroom_port::signals::LineImportanceDetector`] priority signals +
//!    config-supplied keywords).
//! 3. Sort files by total match score; cap to `max_files`.
//! 4. Run [`crate::headroom_port::transforms::adaptive_sizer::compute_optimal_k`] over
...
use crate::headroom_port::ccr::CcrStore;
use crate::headroom_port::signals::{ImportanceContext, LineImportanceDetector};
...
use crate::headroom_port::transforms::adaptive_sizer::compute_optimal_k;
...
/// to a [`crate::headroom_port::signals::KeywordDetector`].
...
impl SearchCompressor {
    pub fn new(config: SearchCompressorConfig) -> Self {
        Self {
            config,
            importance: Box::new(crate::headroom_port::signals::KeywordDetector::new()),
        }
    }
                ...
                if self.config.boost_errors {
                    ...
                    if let Some(category) = signal.category {
                        ...
                        let bump = match category {
                            crate::headroom_port::signals::ImportanceCategory::Error => 0.5,
                            crate::headroom_port::signals::ImportanceCategory::Warning => 0.4,
                            crate::headroom_port::signals::ImportanceCategory::Importance => 0.3,
                            // Categories below aren't part of
                            // PRIORITY_PATTERNS_SEARCH; preserve Python's
                            // behavior of not boosting for them.
                            crate::headroom_port::signals::ImportanceCategory::Security
                            | crate::headroom_port::signals::ImportanceCategory::Markdown => 0.0,
                        };
                        score += bump;
                    }
                }
...
mod tests {
    use super::*;
    use crate::headroom_port::ccr::InMemoryCcrStore;
