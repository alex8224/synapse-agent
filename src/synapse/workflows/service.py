...
from synapse.workflows.coordinator import ApprovalHandler, CallHandler, WorkflowCoordinator
...
from synapse.workflows.recovery import ResumePlan, plan_resume
...
class WorkflowService:
    ...
    async def start(self, run_id: str) -> WorkflowRun:
        """Start (or resume) the worker for one run and return its current record."""
        ...
        self._tasks[run_id] = asyncio.ensure_future(self._drive(run_id, coordinator))
        return run
    ...
    def cancel(self, run_id: str, *, reason: str = "user") -> bool:
        ...
        # has already been asked to stop: the worker is killed here, and the coordinator's
        # loop settles the final status moments later.
        try:
            self.resources.store.set_run_status(
                run_id, WorkflowStatus.CANCELLING, at=self.clock()
            )
