...
def _codex_model_settings(model_settings: dict[str, Any] | None) -> dict[str, Any]:
...
def _prepare_codex_request(request: Any) -> Any:
    ...
    cleaned_settings = _codex_model_settings(model_settings)
...
def build_openai_oauth_compat_middleware() -> AgentMiddleware:
    ...
    class _OpenAIOAuthCompatMiddleware(AgentMiddleware):
        def wrap_model_call(self, request: Any, handler: Any) -> Any:
            return handler(_prepare_codex_request(request))

        async def awrap_model_call(self, request: Any, handler: Any) -> Any:
            return await handler(_prepare_codex_request(request))
